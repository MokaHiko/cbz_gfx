...
public:
	...
	void set() {
		glm::vec3 lookAt = glm::vec3(0.0f, 0.0f, -1.0f) - position;

		forward = glm::normalize(lookAt - glm::vec3(position));
...
public:
	...
	void update() {
		...
		if (cbz::IsKeyDown(cbz::Key::eW)) {
			mCamera->position += mCamera->forward * mDeltaTime * movementSpeed;
		}
