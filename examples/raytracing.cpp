...
class GLTFViewer {
public:
  GLTFViewer(cbz::NetworkStatus netStatus = cbz::CBZNetworkStatus::CBZ_NETWORK_CLIENT) {
    ...
    // --- Blit Pipeline Setup ---
    // Create blit program
    mBlitSH = cbz::ShaderCreate("assets/shaders/blit.spirv");
    ...
    mRaytracingSH = cbz::ShaderCreate("assets/shaders/raytracer.spirv",
                                      "raytracing_shader");
  ...
  void update() {
    ...
    if (cbz::IsKeyDown(cbz::Key::eW)) {
      mCamera->mData.position +=
          mCamera->mData.forward * mDeltaTime * movementSpeed;
    }
    ...
    if (cbz::IsKeyDown(cbz::Key::eD)) {
      mCamera->mData.position +=
          mCamera->mData.right * mDeltaTime * movementSpeed;
    }
...
int main(int argc, char **argv) {
  ...
  GLTFViewer app(argc > 1 ? cbz::CBZNetworkStatus::CBZ_NETWORK_HOST
                       : cbz::CBZNetworkStatus::CBZ_NETWORK_CLIENT);
